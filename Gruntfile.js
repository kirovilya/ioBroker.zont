...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'Kirov Ilya';
    var email     = grunt.option('email')  || 'kirovilya@gmail.com';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new zont name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/zont.png')) {
            fs.renameSync(__dirname + '/admin/zont.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/zont.html')) {
            fs.renameSync(__dirname + '/widgets/zont.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/zont/js/zont.js')) {
            fs.renameSync(__dirname + '/widgets/zont/js/zont.js', __dirname + '/widgets/zont/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/zont')) {
            fs.renameSync(__dirname + '/widgets/zont',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Zont/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Zont'
                        },
                        {
                            match: /Kirov Ilya/g,
                            replacement: author
